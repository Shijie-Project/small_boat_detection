...
def annotation_is_ship_pred_only(annotation: Any) -> bool:
    """
    True if the annotation was generated by this script, i.e. every
    rectanglelabels region is labeled `ship-pred` (and there is at least one).
    Annotations that contain any other label (e.g. a manual `ship` box) are
    left untouched so human work is never deleted.
    """
    ...
    for item in result:
        if _get(item, "type") == "rectanglelabels":
            ...
            labels.extend(_get(value, "rectanglelabels", []) or [])
    return len(labels) > 0 and all(lbl == "ship-pred" for lbl in labels)


def delete_ship_pred_annotations(ls: LabelStudio, tasks: list[Any]) -> int:
    """Delete all previously imported `ship-pred` annotations, return the count deleted."""
    deleted = 0
    for task in tasks:
        for ann in _get(task, "annotations", []) or []:
            if not annotation_is_ship_pred_only(ann):
                continue
            ann_id = _get(ann, "id")
            if ann_id is not None:
                ls.annotations.delete(id=int(ann_id))
                deleted += 1
    return deleted
...
def main(split: Literal["train", "val", "test", "all"]) -> None:
    ...
    # Remove previously imported `ship-pred` annotations so re-running replaces
    # them with the new predictions instead of stacking duplicates. Manual
    # annotations (e.g. `ship` boxes) are never touched.
    deleted_anns = delete_ship_pred_annotations(ls, tasks)
    print(f"Deleted {deleted_anns} old 'ship-pred' annotation(s) from project '{project.title}' (id={project.id})")
    ...
    for image_id, image_info in image_info_by_id.items():
        ...
        # Import as an annotation (not a prediction). The label is set to
        # `ship-pred` so model-generated boxes are distinguishable from the
        # manually drawn `ship` boxes within the annotation layer.
    ...
    print(f"Done. Annotations created for {created} tasks (label='ship-pred').")
